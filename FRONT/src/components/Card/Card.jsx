...
const Card = ({ movie }) => {
	...
	const deleteMovie = id => {
		Swal.fire({
			...
			if (result.isConfirmed) {
				service.deleteData('movies', id).then(response => {
					Swal.fire({
						icon: 'success',
						title: 'Movie deleted',
	...
	const editMovie = movie => {
		return (
			<Modal
				title={'Update Movie From Database' + movie.name}
				type={'add'}
				id={'update_movie'}
				movie={movie}
			/>
			...
			<div className='d-flex flex-column align-items-center'>
				<div className='card mt-4'>
					<div className='card2' onClick={() => showMore(movie.id)}>
						<img src={movie.image} alt='iterstellar' />
					</div>
				</div>
					...
					{srole === 'admin' && (
						<>
							...
							<button
								...
								data-bs-target={`#modal_${movie.id}`} /* onClick={() => {
                                editMovie(movie)
                            }} */
							>
								Update
